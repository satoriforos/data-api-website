...
def handle_get(http_server, database_manager, ip, account, api_key):
    ...
    if len(county_fips) > 0:
        health_insurance_county_conditions = [{
            "column": "county_code",
            "equivalence": "IN",
            "value": county_fips
        }]
        county_totals = database_manager.fetch_by(
            HealthInsuranceCounty(database_manager),
            health_insurance_county_conditions
        )
    else:
        raise ApiParamInvalidZipCodeError
    ...
    output = OrderedDict({
        "health_insurance_ranges": output_data,
        "city": {
            "name": city_name,
            "api_endpoint": '/v1/healthinsurance/city?city={}&state={}'.format(
        ...
        "state": {
            ...
            "api_endpoint": '/v1/healthinsurance/state?code={}'.format(state_code.upper()),
        },
        "country": {
            "name": "United States of America",
            "code": "US",
            "api_endpoint": '/v1/healthinsurance/country?code=US',
        }
    })
