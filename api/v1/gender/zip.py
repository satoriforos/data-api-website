...
def handle_get(http_server, database_manager, ip, account, api_key):
    ...
    if len(county_fips) > 0:
        gender_county_conditions = [{
            "column": "county_code",
            "equivalence": "IN",
            "value": county_fips
        }]
        county_totals = database_manager.fetch_by(
            GenderCounty(database_manager),
            gender_county_conditions
        )
    else:
        raise ApiParamInvalidZipCodeError
    ...
    output = OrderedDict({
        ...
        "city": {
            "name": city_name,
            "api_endpoint": '/v1/gender/city?city={}&state={}'.format(
        ...
        "state": {
            ...
            "api_endpoint": '/v1/gender/state?code={}'.format(state_code.upper()),
        },
        "country": {
            "name": "United States of America",
            "code": "US",
            "api_endpoint": '/v1/gender/country?code=US',
        }
    })
