...
def handle_get(http_server, database_manager, ip, account, api_key):
    ...
    if len(county_fips) > 0:
        age_county_conditions = [{
            "column": "county_code",
            "equivalence": "IN",
            "value": county_fips
        }]
        county_totals = database_manager.fetch_by(
            AgeCounty(database_manager),
            age_county_conditions
        )
    else:
        raise ApiParamInvalidZipCodeError
    ...
    for year in years:
        ...
        if year not in totals:
            if year in raw_output_data:
                ...
                #totals[year] = getattr(
                #    county_total, 'total_{}'.format(year)
                #)
    ...
    output = OrderedDict({
        "age_ranges": output_data,
        "units": "percent",
        "location": {
            "city": {
                "name": city_name,
                "api_endpoint": '/v1/age/city?city={}&state={}'.format(
            ...
            "state": {
                ...
                "api_endpoint": '/v1/age/state?code={}'.format(state_code.upper()),
            },
            "country": {
                "name": "United States of America",
                "code": "US",
                "api_endpoint": '/v1/age/country?code=US',
            }
        },
        "notes": notes
    })
