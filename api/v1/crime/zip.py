...
def handle_get(http_server, database_manager, ip, account, api_key):
    ...
    if len(county_fips) > 0:
        crime_county_conditions = [{
            "column": "county_code",
            "equivalence": "IN",
            "value": county_fips
        }]
        county_totals = database_manager.fetch_by(
            CrimeCounty(database_manager),
            crime_county_conditions
        )
    else:
        raise ApiParamInvalidZipCodeError
    ...
    output = OrderedDict({
        "crime_types": output_data,
        "units": "percent",
        "location": {
            "city": {
                "name": city_name,
                "api_endpoint": '/v1/crime/city?city={}&state={}'.format(
            ...
            "state": {
                ...
                "api_endpoint": '/v1/crime/state?code={}'.format(state_code.upper()),
            },
            "country": {
                "name": "United States of America",
                "code": "US",
                "api_endpoint": '/v1/crime/country?code=US',
            }
        },
        "notes": notes
    })
