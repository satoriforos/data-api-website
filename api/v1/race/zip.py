...
def handle_get(http_server, database_manager, ip, account, api_key):
    ...
    if len(county_fips) > 0:
        race_county_conditions = [{
            "column": "county_code",
            "equivalence": "IN",
            "value": county_fips
        }]
        county_totals = database_manager.fetch_by(
            RaceCounty(database_manager),
            race_county_conditions
        )
    else:
        raise ApiParamInvalidZipCodeError
    ...
    years = [str(x) for x in range(2000, 2011)]
    ...
    output = OrderedDict({
        "race_breakdowns": output_data,
        "units": "percent",
        "location": {
            "city": {
                "name": city_name,
                "api_endpoint": '/v1/race/city?city={}&state={}'.format(
